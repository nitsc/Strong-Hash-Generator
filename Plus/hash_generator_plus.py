...
# 使用盐和 PIPEMD 计算哈希值
def salted_pipemd256_hash(data, salt):
    hasher = RIPEMD.new()
...
def strong_hash(data, salt):
    ...
    sha3256_value = salted_sha3256_hash(sha2512_value, salt)
    
    # 使用 SHA3-256 的结果计算盐化后的 SHA3-512 哈希值
    sha3512_value = salted_sha3512_hash(sha3256_value, salt)
    ...
    pipemd256_value = salted_pipemd256_hash(blake2s_value, salt)
