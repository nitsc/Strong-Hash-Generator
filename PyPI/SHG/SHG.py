...
# 使用 SHA2-512 哈希值
def sha2512_hash(data):
    sha2512 = hashlib.sha512()
    sha2512.update(data.encode('utf-8'))
    return sha2512.hexdigest()

# 使用 SHA3-256 哈希值
def sha3256_hash(data):
    sha3256 = hashlib.sha3_256()
    sha3256.update(data.encode('utf-8'))
    return sha3256.hexdigest()

# 使用 SHA3-512 哈希值
def sha3512_hash(data):
    sha3512 = hashlib.sha3_512()
    sha3512.update(data.encode('utf-8'))
    return sha3512.hexdigest()

# 使用 SHA2-384 哈希值
def sha2384_hash(data):
    sha2384 = hashlib.sha384()
    sha2384.update(data.encode('utf-8'))
    return sha2384.hexdigest()

# 使用 SHA3-384 哈希值
def sha3384_hash(data):
    sha3384 = hashlib.sha3_384()
    sha3384.update(data.encode('utf-8'))
...
# 使用 PIPEMD 哈希值
def pipemd256_hash(data):
    hasher = RIPEMD.new(truncate="256")
    hasher.update(data)
...
# 循环计算 哈希值
def cycle_strong_hash(data, iterations=1024,length=4096):
    salt = generate_salt(length)  # 生成盐
    start_time = time.time()
    # 循环多次哈希计算
    for _ in range(iterations):
        end_hash_value = strong_hash(data, salt)
    end_time = time.time()  # 记录结束时间
    return end_hash_value , end_time - start_time  # 返回
